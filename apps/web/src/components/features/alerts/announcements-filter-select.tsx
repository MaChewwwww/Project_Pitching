...
const FILTER_OPTIONS = [
  ...
  { value: "alert", label: "Alerts & Advisories" },
  { value: "announcement", label: "Announcements" },
] as const;

export interface AnnouncementsFilterSelectProps {
  currentKind?: string;
  className?: string;
}

export function AnnouncementsFilterSelect({
  currentKind = "all",
  className,
}: AnnouncementsFilterSelectProps) {
  const router = useRouter();
  const searchParams = useSearchParams();

  const activeValue = currentKind || "all";
  ...
  const handleValueChange = (value: string) => {
    ...
    router.push(queryString ? `/announcements?${queryString}` : "/announcements");
  };
