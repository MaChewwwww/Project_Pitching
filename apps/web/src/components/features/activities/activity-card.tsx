...
export function ActivityCard({
  ...
  return (
    ...
    >
      <article className="relative flex h-full flex-col overflow-hidden rounded-[20px] border border-neutral-200/90 bg-white shadow-xs transition-all duration-300 hover:-translate-y-1.5 hover:border-primary-600 hover:shadow-xl hover:shadow-black/5">
        ...
        <div className="relative aspect-[16/10] w-full overflow-hidden bg-neutral-100">
          {activity.cover_image ? (
            ...
            <div className="relative flex h-full w-full flex-col justify-between bg-gradient-to-br from-primary-950 via-primary-900 to-neutral-900 p-4 text-white">
              <div
                aria-hidden
                className="absolute -right-8 -bottom-10 size-44 rounded-full border-[20px] border-white/10"
              />
              <div className="relative z-10 flex items-center justify-between">
                <span className="text-[11px] font-bold tracking-wider text-primary-200 uppercase">
                  San Jose Activity
                </span>
              </div>
          ...
          {/* Top Overlays: Category Badge (Left) & Date Glass Badge (Right) */}
          <div className="absolute inset-x-3 top-3 z-10 flex items-center justify-between gap-2">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-black/60 px-2.5 py-1 text-xs font-bold text-white shadow-md backdrop-blur-md">
              <span className="tabular-nums">{phtDayOfMonth(activity.starts_at)}</span>
              <span className="text-primary-200 text-[10px] font-bold tracking-wider uppercase">
        ...
        <div className="flex flex-1 flex-col p-5">
          <h3 className="font-display group-hover:text-primary-800 line-clamp-2 text-lg font-bold leading-snug tracking-tight text-neutral-900 transition-colors">
            {activity.title}
          </h3>
          ...
          <div className="mt-auto flex items-end justify-between gap-3 border-t border-neutral-100 pt-4">
            <div className="flex min-w-0 flex-1 flex-col gap-1 text-xs font-medium text-neutral-500">
              <span className="inline-flex items-center gap-1.5 truncate">
                <CalendarClock aria-hidden className="text-primary-600 size-3.5 shrink-0" />
              ...
              {activity.venue ? (
                <span className="inline-flex items-start gap-1.5 truncate">
                  <MapPin aria-hidden className="text-primary-600 mt-0.5 size-3.5 shrink-0" />
