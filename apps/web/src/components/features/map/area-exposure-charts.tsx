...
import { AlertTriangle, BarChart3, Info, PieChart as PieChartIcon, ShieldAlert, ShieldCheck, Users } from "lucide-react";
...
export function AreaExposureCharts({ areas }: { areas: PublicAreaStat[] }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
      ...
      <Card radius="xl" className="lg:col-span-7 border border-neutral-200/90 shadow-sm flex flex-col justify-between overflow-hidden">
        <CardContent className="p-4 sm:p-5 flex flex-col h-full justify-between gap-4">
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs font-semibold">
                    <span className="font-bold text-neutral-900 flex items-center gap-2">
                      {item.name}
                      <span className="text-[10px] font-medium text-neutral-600 bg-neutral-100 px-1.5 py-0.5 rounded-md">
                        {item.total} HH
                      </span>
                    </span>
          ...
          {/* Footer Note */}
          <div className="pt-2 border-t border-neutral-100 text-[11px] text-neutral-600 flex items-center gap-1.5">
            <Info aria-hidden className="size-3 text-sky-500 shrink-0" />
            <span>Risk classifications derived from household vulnerability criteria (BR-1.6 &amp; M1f).</span>
          </div>
        </CardContent>
      </Card>

      {/* RIGHT COLUMN: San Jose Overall Donut & Risk Summary Cards */}
      <Card radius="xl" className="lg:col-span-5 border border-neutral-200/90 shadow-sm flex flex-col justify-between overflow-hidden">
        <CardContent className="p-4 sm:p-5 flex flex-col h-full justify-between gap-4">
          {/* Header */}
