...
export default function RegisteredCitizensPage() {
  ...
  const columns: ResourceColumn<RegistryMemberOut>[] = [
    { key: "full_name", header: "Citizen", render: (row) => <div className="min-w-48"><p className="font-bold text-neutral-900">{row.full_name}{row.is_head ? <Badge tone="success" className="ml-2">Head</Badge> : null}</p><p className="mt-0.5 text-xs text-neutral-500">{row.relationship_to_head ?? "Household head / relationship not recorded"}</p></div> },
    { key: "household_reference_no", header: "Household Number", render: (row) => <Link className="font-semibold text-emerald-800 hover:underline" href={`/admin/households/${row.household_id}`}>{row.household_reference_no}<span className="mt-0.5 block text-xs font-normal text-neutral-500">{row.household_head_name}</span></Link> },
    { key: "area_name", header: "Area" },
    { key: "sex", header: "Profile", render: (row) => <div className="text-xs text-neutral-600"><span>{row.sex ? row.sex[0].toUpperCase() + row.sex.slice(1) : "Sex not recorded"}</span>{row.birth_date ? <span className="mt-0.5 block">Born {new Date(row.birth_date).toLocaleDateString()}</span> : null}</div> },
