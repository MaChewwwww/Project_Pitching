...
export default function NewAnnouncementPage() {
  ...
  return <div className="mx-auto flex w-full max-w-6xl flex-col gap-8"><PageHeader eyebrow="Article CMS" title="Create" titleAccent="announcement" description="Start with a clear public story. You will add the cover and publish controls on the next screen." action={<Button asChild size="sm" variant="outline"><Link href="/admin/announcements"><ArrowLeft aria-hidden className="size-4" />Back to announcements</Link></Button>} /><section className="rounded-[16px] border border-neutral-200 bg-white p-6 shadow-sm-card sm:p-10"><AnnouncementForm areas={areas} defaultValues={defaults} onSubmit={(values) => create.mutateAsync(values).then(() => undefined)} onCancel={() => router.push("/admin/announcements")} /></section></div>;
}
