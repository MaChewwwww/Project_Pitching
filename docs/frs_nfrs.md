...
| `SAF` | Safety Check-In & Rescue | 19 | 0 | 2 | 0 | 17 | 0 | **11%** |
...
*Overall Active Completion: **108 / 269 requirements (40%)** implemented or in progress across active scope (excluding 12 retired/dropped items).*
...
| ~~FR-SAF-014~~ | ~~Convert emergency record to registration~~ | **Cut, Aug 2026** — post-event admin work, invisible in a five-minute demo. Design retained: `safety.get_unregistered_or_404`/`mark_converted` are already exposed for a future `registry`-owned `POST /admin/households/from-unregistered`, never the reverse (AGENTS.md Section 5) | BR-5.12 | C | ✕ | — |
...
| FR-SAF-018 | Declare an emergency event | Admin names and types an event; at most one active at a time (DB-enforced); superseding a live event is an explicit opt-in, never implicit | — (new, Aug 2026) | M | ◐ | — |
| FR-SAF-019 | End an emergency event | Admin ends the active event; history retained, never deleted | — (new, Aug 2026) | M | ◐ | — |

> **FR-SAF-018/019 did not exist before this pass.** Every other SAF row is scoped to an emergency event (`safety_status.event_id`, `unregistered_person.event_id` are `NOT NULL`), but nothing in the original spec covered creating or ending one — `emergency_event` (owned by `evacuation/models.py`, FR-EVC-*) had no lifecycle endpoint at all, and the one seeded row is permanently `is_active=false`. Without these two requirements, every other SAF row would be unimplementable. Built in `evacuation/service.py` (declare/end/list/`require_active_event`) rather than `safety/`, since evacuation already owns the model. Verified live end-to-end on staging: declare → `GET /public/emergency-events/active` reflects it → end → reflects `null` again.
>
