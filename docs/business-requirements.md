...
**"Profiling for donations" — resolved.** The brainstorm listed this without defining it. It means the donation form itself: donor name, optional contact, and what they intend to give (BR-7.2, BR-7.2a). No account, no donor profile stored beyond the donation record (D-5).
...
**All eleven modules — M0 through M10 — will be built.** The only capabilities excluded from development are SMS notifications and siren/IoT alert units (BR-4.10, BR-4.11), which are documented as future integrations and are not implemented in any form.
