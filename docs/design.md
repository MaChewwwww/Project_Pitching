...
| `base`    | 260ms    | `cubic-bezier(0.2, 0, 0, 1)` — sheets, modals |
...
Authenticated portal entrances use the `slow` cadence with 90ms stagger steps. Press feedback
...
| `PortalLoading`                        | Shape-matched metric, detail, form, timeline, chart, and map skeletons plus `DataSurfaceLoading` for large portal workspaces. A query replaces only its own region while fetching; empty and error remain distinct outcomes.                                                                                                                                          |
| `WaterSpinner`                         | The public site's loading indicator: a droplet falling into a rippling surface inside two counter-precessing rings. Green — a spinner in the hazard ramp (Section 3.4) would read as an active warning. Pure CSS on static markup, so it renders inside a Server Component's `<Suspense fallback>` and ships no JavaScript. Portal callers opt into its calmer tempo. |
